...
import { updateGraphNodesAndEdges } from "../updateGraphWorker";
...
export const DoiNetwork: React.FC<DoiNetworkProps> = ({ doi, n }) => {
	...
	useEffect(() => {
		...
		const updateGraphData = async () => {
			const data = { doi, citations, references, isDarkMode: darkMode };
			const { newNodes, newEdges } = await updateGraphNodesAndEdges(data);
			setNodes(newNodes);
			setEdges(newEdges);
			setComputing(false);
		};
